...
# -*- coding: utf-8 -*-
"""
Created on Sun Aug 25 15:57:47 2019
...
def train_weights(train,rate,count,size,test):
    ...
    for row in train:
        ...
        if(val==row[size-1]):
            count=count+1;
        #print("Expected {} and predicted {}".format(row[size-1],val))
    #print(weight)
    count=0
    ll=0
    for row in test:
...
def main():
    ...
    #print(rows)
    for i in range(len(rows)):
        if rows[i][0]=='Iris-setosa':
            rows[i][0]=1
        elif rows[i][0]=='Iris-versicolor':
            rows[i][0]=0
    ...
    for i in range(len(rows)):
        for j in range(0,len(rows[i])):
           if rows[i][j]=='Iris-versicolor':
                rows[i][j]=0
                
           elif rows[i][j]=='Iris-setosa':
                rows[i][j]=1
           else:
